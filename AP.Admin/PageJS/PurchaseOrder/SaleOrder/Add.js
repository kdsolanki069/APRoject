...
$('#btnAddSaleOrder').click(function () {
    var status = validationdata();
    if (status) {
        var UserObject = GetSaleOrderValue();
        InsertUpdateSaleOrder(UserObject);
    }
    else {
...
function GetSaleOrderValue() {
    var UserObject = GetSaleOrderData();
    return UserObject;
}
...
function InsertUpdateSaleOrder(UserObject) {
    ...
    function OnSuccess(data) {
        if (data != -1) {
            if (data.Status == "True") {
                ...
                $('#txttext').text('Sale Order Detail Add Sucessfully');
