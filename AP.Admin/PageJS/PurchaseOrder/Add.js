...
$('#btnAddPurchase').click(function () {
    ...
    if (status) {
        var UserObject = GetPurchaseOrderValue();
        InsertUpdatePurchaseOrder(UserObject);
    }
    else {
...
function GetPurchaseOrderValue() {
    var UserObject = GetPurchaseOrderData();
    return UserObject;
}
...
function InsertUpdatePurchaseOrder(UserObject) {
    ...
    function OnSuccess(data) {
        if (data != -1) {
            if (data.Status == "True") {
                ...
                $('#txttext').text('Purchase Order Detail Add Sucessfully');
